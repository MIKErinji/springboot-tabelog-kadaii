...
public interface RestaurantRepository extends JpaRepository<Restaurant, Integer> {
	...
	@Query("SELECT r FROM Restaurant r " +
			...
			"ORDER BY r.createdAt DESC")
	Page<Restaurant> findByCategoryIdOrderByCreatedAtDesc(@Param("categoryId") Integer categoryId, Pageable pageable);
	...
	@Query("SELECT r FROM Restaurant r " +
			...
			"WHERE cr.category.id = :categoryId " +
			"ORDER BY r.lowestPrice ASC")
	Page<Restaurant> findByCategoryIdOrderByLowestPriceAsc(@Param("categoryId") Integer categoryId, Pageable pageable);

	//指定価格以下の店舗（作成日時順）

	Page<Restaurant> findByLowestPriceLessThanEqualOrderByCreatedAtDesc(@Param("price") Integer price,
			Pageable pageable);

	//指定価格以下の店舗（最低価格順）

	Page<Restaurant> findByLowestPriceLessThanEqualOrderByLowestPriceAsc(@Param("price") Integer price,
			Pageable pageable);
